      ...
      <div className=' w-full flex justify-between items-center gap-2 '>
        <div className='flex items-center'>
          <button
            ...
            className='w-6 h-6 flex items-center justify-center rounded border border-zinc-600 text-zinc-400 hover:text-zinc-100 hover:border-zinc-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-sm'
          >
            −
          </button>
        ...
        {onDownload && (
          <button
            type='button'
            onClick={onDownload}
            className='rounded px-3 py-1.5 text-sm font-medium bg-[#187676] text-white hover:bg-indigo-500 transition-colors'
          >
            Download all
          </button>
        )}
      </div>
      <div className='flex flex-col items-start gap-6' style={{ zoom }}>
        {cards.map((card) => {
          const metric = metrics.find(
            (m) =>
              card.id.includes(`_${slug(m.label)}_`) ||
              card.id.endsWith(`_${slug(m.label)}`),
          )
          return (
            <div
              key={card.id}
              id={`card-${card.id}`}
              className='border border-zinc-700 rounded-lg overflow-hidden shadow-sm shadow-zinc-950'
            >
              ...
              <div className='px-3 py-1.5 bg-zinc-800 border-t border-zinc-700 flex items-center gap-2 flex-wrap'>
                <span
                  className='text-xs text-zinc-500 truncate max-w-32'
                  title={`${card.id}.svg`}
                >
                  {card.label}
                </span>
                  ...
                  <>
                    ...
                    {(metric.chartType === 'bar' ||
                      ...
                      <button
                        title={
                          metric.pivot
                            ? 'Flipped (click to reset)'
                            : 'Normal (click to flip)'
                        }
                        onClick={() =>
                          onDatasetsJsonChange(
                            patchMetricField(
